            ...
            for i in range(len(model_state_msg.name)):
                for model in self.model_list:
                    if model_state_msg.name[i] == model:
                        #Get model pose from Gazebo message -- Modified coordinates
                        self.model_pub[model]["pose_stamped_msg"].pose.position.x = model_state_msg.pose[i].position.x
                        self.model_pub[model]["pose_stamped_msg"].pose.position.y = model_state_msg.pose[i].position.y
                        self.model_pub[model]["pose_stamped_msg"].pose.position.z = model_state_msg.pose[i].position.z
